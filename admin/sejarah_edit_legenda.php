...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $gambar = $legenda['gambar'];
    if (isset($_FILES['gambar']) && $_FILES['gambar']['name']) {
        $ext = pathinfo($_FILES['gambar']['name'], PATHINFO_EXTENSION);
        $namaFile = 'legenda_' . time() . '.' . $ext;
        $target = '../assets/' . $namaFile;
        if (move_uploaded_file($_FILES['gambar']['tmp_name'], $target)) {
            $gambar = $namaFile;
        }
    }
    mysqli_query($konek, "UPDATE sejarah_desa SET konten='$konten', gambar='$gambar' WHERE bagian='legenda'");
    header('Location: sejarah.php');
    exit();
}
?>
<!-- admin/sejarah_edit_legenda.php -->
<!DOCTYPE html>
<html lang="id">
  <head>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <title>Edit Legenda Desa - Desa Pacing</title>
  ...
  <body class="hold-transition sidebar-mini">
      ...
      <div class="content-wrapper">
        <div class="content-header">
          <div class="container-fluid">
            <div class="row mb-2">
              <div class="col-sm-6">
                <h1 class="m-0">Edit Legenda Desa</h1>
              </div>
            </div>
          </div>
        </div>
              ...
              <div class="card-body">
                <form method="post" enctype="multipart/form-data">
                  <div class="form-group">
                    <label>Isi Legenda</label>
                    <div id="editor-deskripsi" style="height: 250px">
                      <?= htmlspecialchars_decode($legenda['konten'] ?? '') ?>
                    </div>
                  ...
                  <div class="form-group">
                    ...
                    <?php if ($legenda && $legenda['gambar']) { ?>
                      <div class="mt-2">
                        <img src="../assets/<?= $legenda['gambar'] ?>" alt="Gambar Legenda" style="max-width:200px;" />
                      </div>
                    <?php } ?>
                  </div>
    ...
    <script>
      ...
      <?php if ($legenda && $legenda['konten']) { ?>
        quillDeskripsi.root.innerHTML = <?= json_encode($legenda['konten']) ?>;
      <?php } ?>
