...
$data = mysqli_fetch_assoc(mysqli_query($konek, "SELECT * FROM pengumuman WHERE id=$id"));
if (!$data) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $gambar = $data['gambar']; // Gunakan gambar yang sudah ada
    ...
    if ($judul && $tanggal && $konten) {
        mysqli_query($konek, "UPDATE pengumuman SET judul='$judul', tanggal='$tanggal', konten='$konten', gambar='$gambar' WHERE id=$id");
        header('Location: pengumuman.php');
        exit();
    } else {
...
<head>
    ...
    <title>Edit Pengumuman - Desa Pacing</title>
                    ...
                    <div class="card-body">
                        ...
                        <form method="post" enctype="multipart/form-data">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" id="judul" name="judul" value="<?= htmlspecialchars($data['judul']) ?>" placeholder="Masukkan judul pengumuman" required />
                            </div>
                            <div class="form-group">
                            ...
                            <div class="form-group">
                                ...
                                <?php if (!empty($data['gambar'])) { ?>
                                    <div class="mb-2">
                                        <img src="../assets/<?= $data['gambar'] ?>" alt="Gambar Saat Ini" class="img-thumbnail" style="max-width: 200px; max-height: 200px;">
                                        <p class="text-muted">Gambar saat ini: <?= $data['gambar'] ?></p>
                                    </div>
                                <?php } ?>
                                <input type="file" class="form-control" id="gambar" name="gambar" accept="image/*" />
                                <small class="form-text text-muted">Format yang diizinkan: JPG, JPEG, PNG, GIF. Maksimal 2MB. Kosongkan jika tidak ingin mengubah gambar.</small>
                            </div>
                            <div class="form-group">
